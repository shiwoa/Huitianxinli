...
import com.yizhilu.community.utils.DisplayUtil;
...
public class GroupImageView extends ViewGroup implements View.OnClickListener {
	...
	private void init(Context context) {
		...
		picOptions = new DisplayImageOptions.Builder().cacheInMemory(true).cacheOnDisc(true)
				.showImageOnLoading(R.drawable.pic_bg).showImageForEmptyUri(R.drawable.pic_bg)
				.showImageOnFail(R.drawable.pic_bg).build();
	}
	...
	public void setPics(List<String> picUrls) {
		...
		if (picUrls == null || picUrls.size() == 0 || TextUtils.isEmpty(picUrls.get(0))) {
	...
	private void displayGroupImageView() {
		int maxWidth = DisplayUtil.getWindowWidth(mContext) - DisplayUtil.dip2px(mContext, 10 * 2);
